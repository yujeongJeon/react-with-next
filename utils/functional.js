const functional = (function() {
    ...
    const flat = (coll) => coll.reduce((a, b) => a.concat(b), []);
    ...
    const reduceB = curry(function(f, coll, acc) {
      const iter = collIter(coll);
      return go(arguments.length == 2 ? iter.next().value : acc, function recur(
        acc
      ) {
        var cur;
    ...
    const each = curry((f, coll) =>
      go(
        reduce((_, a) => f(a), coll, null),
        (_) => coll
      )
    );
    ...
    const baseMFIter = (f1, f2) =>
      curry((f, iter, acc = hasIter(iter) ? [] : {}) =>
        Array.isArray(acc)
    ...
    const filter = curry((f, coll) =>
      hasIter(coll)
        ? filterIter(f, coll, [])
        : filterIter(f, entriesIter(coll), {})
    );
  
    const countBy = curry((f, coll) =>
      reduce((counts, a) => incSel(counts, f(a)), coll, {})
    );
  
    const groupBy = curry((f, coll) =>
      reduce((group, a) => pushSel(group, f(a), a), coll, {})
    );
  
    const indexBy = curry((f, coll) =>
    ...
    const tap = (f, ...fs) => (arg, ...args) =>
      go(f(arg, ...args), ...fs, (_) => arg);
      ...
      return new Promise(function recur(resolve) {
        ...
        while ((cur = iter.next()) && !cur.done) {
          ...
          go(
            ...
            (_) =>
              i == ++j ? (iter.remain ? recur(resolve) : resolve()) : undefined
          );
        }
      });
    });
    ...
    const defaults = baseExtend(
      tap((obj, kv) => has(kv[0], obj) || set2(obj, kv))
    );
  
    const uniqueBy = curry((f, coll) => {
      const evds = new Set();
      return filter(
        (a) => go(a, f, (evd) => (evds.has(evd) ? false : evds.add(evd))),
        coll
      );
    });
    ...
    const equals = curry(function equals(a, b) {
      return a === b;
    });
  ...
  Object.assign(!!isClient ? window : global, functional);
  
  const common = require("./common");
  Object.assign(!!isClient ? window : global, common, functional);