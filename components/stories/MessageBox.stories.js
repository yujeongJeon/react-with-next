...
export const TextMessage = () => {
    ...
    const contents = text("contents", "텍스트 채팅 예시입니다.\n뭔가 길게 적어야 할 거 같은데 뭐라고 해야할지 모르겠습니다 ^_^;;;");
    const dateTime = text("dateTime", timestamp);
    return <MessageBox type={ type } contents={ contents } dateTime={ dateTime } btnOnClick={ action("btnOnClick") } />;
};
...
export const ImageMessage = () => {
    ...
    const contents = text("contents", "http://localhost:3001/images/pexels-photo-617278.jpeg");
    const dateTime = text("dateTime", timestamp);
    return <MessageBox type={ type } contents={ contents } dateTime={ dateTime } btnOnClick={ action("btnOnClick") } imgOnClick={ action("imgOnClick") } />;
};
...
export const MessageWithButtons = () => {
    ...
    const dateTime = text("dateTime", timestamp);
    const buttons = array("buttons", [{ name: "버튼 1" }, { name: "버튼 2" }, { name: "버튼 3" }]);

    return <MessageBox type={ type } contents={ contents } dateTime={ dateTime } buttons={ buttons } btnOnClick={ action("btnOnClick") } imgOnClick={ action("imgOnClick") } />;
};
